...
let test_file_to_prev_hunk () =
  let first_file_hunk : TuiTypes.hunk =
    { starting_line = 1; context_snippet = None; visibility = Collapsed; lines = [] }
  in
  let first_file : TuiTypes.file =
    { path = FilePath "src/main"; visibility = Expanded; hunks = [ first_file_hunk ] }
  in
  let second_file : TuiTypes.file =
    {
      path = FilePath "src/test";
  ...
  let expected =
    TuiModel.Hunk
      (Zipper.Zip ([], first_file, [ second_file ]), Zipper.Zip ([], first_file_hunk, []))
  in
  check tui_model_testable "same TUI models" expected prev_model

let test_file_to_prev_line () =
...
let test_first_file_to_last_file () =
  let first_file : TuiTypes.file =
    {
      path = FilePath "src/main";
      visibility = Collapsed;
...
let test_first_file_to_last_file_last_hunk () =
  let first_file : TuiTypes.file =
    {
      path = FilePath "src/main";
      visibility = Collapsed;
...
let test_first_file_to_last_file_last_hunk_last_line () =
  let first_file : TuiTypes.file =
    {
      path = FilePath "src/main";
      visibility = Collapsed;
...
let prev_suite =
  [
    ("moves the cursor from a file to the previous file", `Quick, test_file_to_prev_file);
    ("moves the cursor from a file to the previous file's last hunk", `Quick, test_file_to_prev_hunk);
    ( "moves the cursor from a file to the previous file's last hunk's last line",
      `Quick,
      test_file_to_prev_line );
    ("moves the cursor from a hunk to its file", `Quick, test_hunk_to_file);
    ("moves the cursor from a hunk to the previous hunk", `Quick, test_hunk_to_prev_hunk);
    ("moves the cursor from a hunk to the previous hunk's last line", `Quick, test_hunk_to_prev_line);
    ("moves the cursor from a line to its hunk", `Quick, test_line_to_hunk);
    ("moves the cursor from a line to the previous line", `Quick, test_line_to_prev_line);
    ("moves the cursor from the first file to the last file", `Quick, test_first_file_to_last_file);
    ( "moves the cursor from the first file to the last file's last hunk",
      `Quick,
      test_first_file_to_last_file_last_hunk );
    ( "moves the cursor from the first file to the last file's last hunk's last line",
      `Quick,
      test_first_file_to_last_file_last_hunk_last_line );
  ]
